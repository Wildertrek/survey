...
- **Cross-model convergence: 7.0 → 8.6 → 9.4 models per query.** The atlas is also a search engine — query any trait and retrieve nearest neighbors across all 44 models. In the baseline 1,536-dim index, a typical query retrieves hits from 7.0 independent models. Upgrade to 3,072 dimensions and that jumps to 8.6 (a 23% increase) because the higher-dimensional space gives similar-but-distinct constructs more room to separate, which paradoxically makes genuinely related ones easier to find. Add augmented training data and it reaches 9.4 models across 4.5 of the 7 categories — meaning a single trait description pulls back related constructs from roughly half the atlas's categories and nearly a quarter of all 44 models. That is the cross-tradition retrieval the atlas was built for.
