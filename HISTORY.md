...
The taxonomy was not imposed from the top down. It emerged from the models themselves as we kept finding more of them. You can see it reflected in the folder structure under [`atlas/`](atlas/).
...
**2. Classification.** For each model, we trained four classifiers — Random Forest (RF), Linear SVC, Logistic Regression, and k-Nearest Neighbors (kNN) — to predict which factor a trait belongs to, given its embedding vector. Most score above 95% on their own training data (cross-validation). On novel items, the ranking reverses: Linear SVC (74.7%) and kNN (73.5%) substantially outperform RF (60.4%), and on human-authored items, kNN leads at 83.5% (oracle 86.8%). All 176 classifiers ship in this repository.
...
- **418 items from 22 published instruments** (BFI-44, IPIP-50, HEXACO-60, GAD-7, Short Dark Triad, NARQ, etc.) classified through the atlas.
- **RF accuracy: 68.9%** — 10.3 percentage points higher than on LLM-generated items. Published psychometric items are designed to load cleanly on single factors, and the classifiers pick up on that. Under a four-way classifier comparison (RF, Linear SVC, LR, kNN), kNN achieves 83.5% and the best-per-model oracle reaches 86.8% (Friedman chi2 = 22.4, p < 0.001).
...
| RQ11 | Can the classifiers handle items written by human psychometricians? | Yes. 68.9% RF accuracy on 418 items from 22 published instruments (+10.3pp vs LLM items). kNN: 83.5%, oracle: 86.8%. |
