#!/usr/bin/env python3
"""Generate Colab notebook for expert evaluation (no-install alternative to Streamlit app)."""
...
def make_notebook():
    ...
    code([
        ...
        "        f'<div style=\"border:1px solid #ccc;padding:12px;margin:8px 0;border-radius:8px;background:#fafafa\">'\n",
        "        f'<h3 style=\"margin-top:0\">{row[\"category\"]} ({row[\"n_models\"]} models)</h3>'\n",
        ...
        "        f'</details>'\n",
        "        f'<p style=\"background:#fffde7;padding:8px;border-radius:4px;margin-top:8px\">'\n",
    ...
    code([
        ...
        "            f'<h3 style=\"margin-top:20px;border-bottom:2px solid #1976d2;padding-bottom:4px\">'\n",
        ...
        "    table = '<table style=\"font-size:12px;border-collapse:collapse;margin:4px 0\">'\n",
        "    table += '<tr style=\"background:#e3f2fd\"><th style=\"padding:3px 8px\">Adjective</th>'\n",
        ...
        "        while len(parts) < 4:\n",
        "            parts.append('')\n",
        "        table += f'<tr><td style=\"padding:2px 8px\">{parts[0]}</td>'\n",
        ...
        "        f'<b>{row[\"factor\"]}</b> '\n",
        "        f'<span style=\"color:#666\">({len(entries)} sample entries from {row[\"n_schema_rows\"]} rows)</span>'\n",
        "        f'{table}</div>'\n",
        "    ))\n",
        "\n",
    ...
    code([
        ...
        "\n",
        "# Create all widgets\n",
        "item_display = widgets.Output()\n",
        "factor_defs = widgets.Output()\n",
        "\n",
        "factor_select = widgets.Dropdown(\n",
        "    options=[''],\n",
        "    value='',\n",
        "    description='Factor:',\n",
        ...
        ")\n",
        "confidence_label = widgets.HTML('<span style=\"font-size:11px;color:#666\">1=guessing, 5=certain</span>')\n",
        "\n",
        ...
        "def save_current():\n",
        "    idx = task1_state['idx']\n",
        "    if factor_select.value:\n",
        "        task1_responses[idx] = {\n",
        "            'factor': factor_select.value,\n",
        "            'confidence': confidence.value,\n",
        "            'notes': notes_input.value\n",
        "        }\n",
        "        items_df.at[idx, 'expert_factor_assignment'] = factor_select.value\n",
        ...
        "        f'<span style=\"color:#666\">({done} answered, {pct:.0f}%)</span>'\n",
        "        f'</div>'\n",
        "    )\n",
        "\n",
        "    # Update item display\n",
        ...
        "    item_display.clear_output()\n",
        "    with item_display:\n",
        "        display(HTML(\n",
        "            f'<div style=\"border:1px solid #ccc;padding:12px;border-radius:8px;background:#fafafa\">'\n",
        "            f'<div style=\"display:flex;justify-content:space-between\">'\n",
        "            f'<span style=\"color:#666\">{row[\"item_id\"]}</span>'\n",
        "            f'<span style=\"color:#666\">{row.get(\"source_type\", \"unknown\")}</span>'\n",
        "            f'</div>'\n",
        "            f'<p style=\"font-size:16px;background:#e3f2fd;padding:10px;border-radius:4px;margin:8px 0\">'\n",
        "            f'\\\"{row[\"item_text\"]}\\\"</p>'\n",
        ...
        "    factor_select.options = [''] + factors\n",
        "\n",
        "    # Show factor definitions\n",
        "    guide = factor_guide.get(row['model'], {})\n",
        "    factor_defs.clear_output()\n",
        "    if guide:\n",
        "        with factor_defs:\n",
        "            lines = []\n",
        "            for f in factors:\n",
        "                desc = guide.get(f, '')\n",
        "                if desc:\n",
        "                    lines.append(f'<b>{f}:</b> {desc}')\n",
        "            if lines:\n",
        "                display(HTML(\n",
        "                    '<details style=\"margin:8px 0\">'\n",
        "                    '<summary style=\"cursor:pointer;color:#1976d2\">'\n",
        "                    'Show factor definitions</summary>'\n",
        "                    '<div style=\"font-size:13px;padding:8px;background:#f5f5f5;'\n",
        ...
        "        try:\n",
        "            factor_select.value = resp['factor']\n",
        "        except Exception:\n",
        "            factor_select.value = ''\n",
    ...
    notebook = {
        ...
        "metadata": {
            "colab": {
                "provenance": [],
                "name": "Personality Atlas - Expert Review"
            },
            "kernelspec": {
                "name": "python3",
...
if __name__ == "__main__":
    nb = make_notebook()
    out_path = "expert_evaluation_colab.ipynb"
