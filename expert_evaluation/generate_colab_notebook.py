...
def make_notebook():
    ...
    code([
        ...
        "\n",
        "# Maps display label -> raw factor name for current item\n",
        "factor_label_map = {}\n",
        "\n",
        "factor_select = widgets.RadioButtons(\n",
        "    options=[''],\n",
        "    value=None,\n",
        "    description='Factor:',\n",
        ...
        "def save_current():\n",
        "    idx = task1_state['idx']\n",
        "    if factor_select.value is None:\n",
        "        return\n",
        "    raw_factor = factor_label_map.get(factor_select.value, factor_select.value)\n",
        "    if raw_factor:\n",
        "        task1_responses[idx] = {\n",
        "            'factor': raw_factor,\n",
        ...
        "        }\n",
        "        items_df.at[idx, 'expert_factor_assignment'] = raw_factor\n",
        ...
        "    factor_label_map.clear()\n",
        "    labeled_options = []\n",
        "    for f in factors:\n",
        "        desc = guide.get(f, '')\n",
        "        label = f'{f} \\u2014 {desc}' if desc else f\n",
        "        factor_label_map[label] = f\n",
        "        labeled_options.append(label)\n",
        "    factor_select.options = labeled_options\n",
        ...
        "        # Find the label matching the saved factor\n",
        "        match = [lbl for lbl, raw in factor_label_map.items() if raw == resp['factor']]\n",
        "        try:\n",
        "            factor_select.value = match[0] if match else None\n",
        "        except Exception:\n",
        "            factor_select.value = None\n",
